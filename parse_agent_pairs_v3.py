...
# Build a map of filename to absolute path for all .txt prompt files
# so we can find them even if the python file hardcodes the wrong directory
prompt_map = {}
for root, _, files in os.walk(prompts_dir):
    for f in files:
        if f.endswith('.txt'):
            prompt_map[str(f)] = os.path.abspath(os.path.join(root, f))
    ...
    for root, _, files in os.walk(agents_dir):
        for f in files:
            if f.endswith('.py') and f not in ('__init__.py', 'agent.py', 'base_ontology_agent.py'):
                ...
                # Extract all embedded .txt files that are passed to load_prompt
                sys_prompts = []
                # Use re.findall with DOTALL to find all load_prompt(...) calls
                # Then grab the string inside it
                matches = re.finditer(r'load_prompt\s*\(\s*(?:r?f?["\'])(.*?\.txt)["\']\s*\)', content, re.DOTALL)
                for match in matches:
                    prompt_path = match.group(1).replace('\\\\', '\\').replace('\\', '/')
                    filename = os.path.basename(prompt_path)
                    
                    # Resolve real path
                    actual_path = prompt_map.get(filename)
                    if actual_path and os.path.exists(actual_path):
                        with open(actual_path, 'r', encoding='utf-8') as sp_file:
                            sys_prompts.append(sp_file.read())
                    else:
                        sys_prompts.append(f"Could not read system prompt: {filename} from {prompt_path}")
                ...
                for i, line in enumerate(lines):
                    if (re.search(r'^\s*(user_msg|user_message|user_prompt|prompt)\s*=', line)):
                        ...
                        if line.count('(') > line.count(')') or line.count('"""') % 2 != 0 or line.count("'''") % 2 != 0:
                            ...
                            j = i + 1
                            while j < len(lines) and (parens > 0 or triple_quotes or single_quotes):
