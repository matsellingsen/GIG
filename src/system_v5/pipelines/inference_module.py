    ...
    def run(self) -> dict:
        while True:
            ...
            if fetched_relevant_info == "noPrimaryEntityFound":
                ...
                continue
            elif fetched_relevant_info == "noComparativeObjectFound":     
                print(f"could not find a resolved object in the KB that fits for the extracted object {question_info.get('object').get('value')} in the comparative question. Cannot proceed with answer generation.") 
                print("Please try again with a different question.")
                continue
            ...
            mapped_answer_entity_answer = map_answer_to_context(answer=answer_text, context=entity_context)
            mapped_entity_merged = merge_mappings(mapped_reasoning_entity_answer, mapped_answer_entity_answer)

            mapped_reasoning_object_answer = map_answer_to_context(answer=reasoning_text, context=object_context) if object_context is not None else None
            mapped_answer_object_answer = map_answer_to_context(answer=answer_text, context=object_context) if object_context is not None else None
            mapped_object_merged = merge_mappings(mapped_reasoning_object_answer, mapped_answer_object_answer) if object_context is not None else None
